...
heterogeneousMLE_vary <- function(x, d, y, ninits){
  n <- length(y)
  alpha.dim <- ncol(x)
  x.null <- x[d == 0, ]
  y.null <- y[d == 0]
  ...
  lambda <- lambda.set[index]
  sigma <- sigma.set[index]
  parm <- list(pi = pi, alpha=alpha, beta=beta, lambda=lambda, sigma=sigma)
  ll <- ll.set[index]
...
shrinkageMLE_nonEM <- function(x, d, z, y, p, ninits=5, theta.initial, epsilon = 1e-08, maxit = 500, 
                         epsilon.short = 1e-02, maxit.short = 100){
  ##Updated on January 30th
  n <- length(y)
  alpha.dim <- ncol(x)
