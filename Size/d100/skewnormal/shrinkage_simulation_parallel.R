...
#rownames(summary_matrix) <- c("100", "250", "500", "750", "1000")
rownames(summary_matrix) <- c("1000")
colnames(summary_matrix) <- c("rfreq equal", "rfreq SLR", "adj. 95 equal", "adj. 95 SLR")
...
for(i in 1:length(sample_size)) {
  ...
  z_vec <- double(n*nreps*(gamma.dim - 1))
  index <- 1
  for(iter in 1:(n / 250)) {
    vec <- rsn(250*nreps*(gamma.dim - 1), xi = location, omega = scale, alpha = shape)
    z_vec[index:(index + 250*nreps*(gamma.dim - 1) - 1)] <- vec
    index <- index + 250*nreps*(gamma.dim - 1)
  }
  rm(list=c("vec"))
  z_set <- array(z_vec, dim = c(n, gamma.dim - 1, nreps))
  rm(list=c("z_vec"))
  ...
  summary_matrix[i, ] <- c(rfreq_equal, rfreq, quantile(LRT_equal, 0.95), quantile(SLR, 0.95))
  
  save(summary_matrix, file = "summary1000.RData")
  print(i)
